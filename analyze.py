...
def read_csv_with_encoding(file_path):
    ...
    for encoding in encodings:
        try:
            ...
            print(f"Successfully read file using {encoding} encoding: {os.path.basename(file_path)}")
            return df
...
def analyze_grade_distribution():
        ...
        try:
            ...
            if grade_col is not None:
                ...
                if len(invalid_grades) > 0:
                    print(f"  Warning: Found {len(invalid_grades)} invalid Grade values: {sorted(invalid_grades.unique())}")
                ...
                print(f"File {os.path.basename(file_path)} does not contain 'Grade' column")
    ...
    axes[1,0].hist(all_grades_series, bins=np.arange(all_grades_series.min()-0.5, 
                                                     all_grades_series.max()+1.5, 1),
    ...
    # 4. Box plot
    axes[1,1].boxplot(all_grades_series, patch_artist=True,
    ...
    if len(file_stats) > 1:
        ...
        for stat in file_stats:
            ...
            for grade in all_unique_grades:
                ...
                percentage = count / stat['valid_grades'] * 100 if stat['valid_grades'] > 0 else 0
                row.append(percentage)
            matrix_data.append(row)
            file_labels.append(stat['file'].replace('SM16-v-SM17_', '').replace('.csv', ''))
        ...
        plt.tight_layout()
        plt.savefig('plots/grade_distribution_heatmap.png', dpi=300, bbox_inches='tight')
        plt.show()
